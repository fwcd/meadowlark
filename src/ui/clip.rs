...
impl Clip {
    pub fn new(
        ...
        Self { clip_start, clip_id, track_id }.build2(cx, move |cx| {
            if cx.data::<ClipData>().is_none() {
                // Create some internal slider data (not exposed to the user)
                ClipData { dragging: false, start_time: clip_start }.build(cx);
            } else {
                // FIX ME
                let clip_data = cx.data::<ClipData>().unwrap();
                ClipData { dragging: clip_data.dragging, start_time: clip_start }.build(cx);
            }
