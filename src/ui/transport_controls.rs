    ...
    VStack::new(cx, |cx|{
        ...
        HStack::new(cx, |cx|{
            ...
            Binding::new(cx, StateSystem::ui_state.then(UiState::timeline_transport).then(TimelineTransportUiState::playhead), |cx, playhead|{
                ...
                Label::new(cx, &format!("{}.{}.{}",  (beats/ 4.0) as i32 + 1, (beats as i32 % 4) + 1, ((beats * 4.0) as i32 % 4) + 1))
                    .width(Pixels(50.0))
            ...
            Button::new(cx, |cx| cx.emit(AppEvent::Stop), |cx|{
                Label::new(cx, ICON_STOP).font("icons").font_size(24.0).width(Stretch(1.0)).child_space(Stretch(1.0));
            }).width(Pixels(30.0)).height(Pixels(30.0)).background_color(Color::rgba(255, 0, 0, 0));
        }).height(Pixels(30.0));
